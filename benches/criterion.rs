...
use backtrack::problems::{BinQueens, NQueens, TotalSum};
...
fn bench_bin_queens(n: usize) -> usize {
    let n_queens = BinQueens::new(n);
...
fn criterion_bench_bin_queens(c: &mut Criterion) {
    c.bench_function("bin-queens 9", |b| b.iter(|| bench_bin_queens(black_box(9))));

    let mut g_small = c.benchmark_group("smaller-sample-size");
    g_small.sample_size(10);
    g_small.bench_function("bin-queens 10", |b| b.iter(|| bench_bin_queens(black_box(10))));
    g_small.bench_function("bin-queens 11", |b| b.iter(|| bench_bin_queens(black_box(11))));
}
